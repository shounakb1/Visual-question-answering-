...
def train(learning_rate, learning_rate_decay, dropout_rate, mini_batch_size, epochs, optimizer, random_seed, model_directory, model_filename, log_directory,continue_training):
    ...
    if continue_training:
        ...
        validation_idx = [shuffled_idx[k: k + mini_batch_size] for k in range(4000, 4800, mini_batch_size)]
        lowest_loss=0
...
#     y_test = np.array(y[800:1000])
    x_test=[]
    y_test=[]
    mini_batch_idx = [shuffled_idx[k] for k in range(10000, 10200)]
...
def main():
    ...
    mini_batch_size_default = 200
    epochs_default = 30
    optimizer_default = 'Adam'
    random_seed_default = 0
    model_directory_default = 'model_large'+str(learning_rate_default)+str(learning_rate_decay_default)+str(dropout_rate_default)+'(lr,deacy,dropout)'
